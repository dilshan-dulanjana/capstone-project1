  ...
  const handleUpdate = async (id) => {
    try {
      const response = await axios.patch(
        `http://localhost:8070/api/travelagent/${id}`,
        formData
      );
      setTravelAgents(
        travelAgents.map((agent) => (agent._id === id ? response.data : agent))
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8070/api/travelagent",
        formData
      );
          ...
          <Row>
            ...
            <Form inline onSubmit={handleSearch}>
              <FormControl
                ...
                style={{ display: "inline-block", width: "auto", alignContent:"center" }}
              />
              <Button
                type="submit"
                variant="primary"
                        ...
                        {editableTravelAgent === travelAgent._id ? (
                          <>
                            ...
                            <Button
                              variant="primary"
                              className="mr-2"
                              onClick={() => handleUpdate(travelAgent._id)}
                            >
                              Update
                            </Button>
                            <Button
                              variant="secondary"
                              onClick={() => setEditableTravelAgent(null)}
                            >
                              Cancel
                            </Button>
                          </>
                          ...
                          <>
                            ...
                            <Card.Text>
                              <b>Address:</b> {travelAgent.address}
                              <br />
                              <b>Website:</b>{" "}
                              <a
                                href={travelAgent.web}
                                target="_blank"
                              ...
                              <br />
                              <b>Registration No:</b>{" "}
                              {travelAgent.registrationNo}
                              <br />
                              <b>License No:</b> {travelAgent.licenseNo}
                              <br />
                              <b>Validity Status:</b>{" "}
                              {travelAgent.validityStatusOfLicense}
                              <br />
                              <b>District:</b> {travelAgent.district}
                              <br />
